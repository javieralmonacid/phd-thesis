...
rats \cite{GillespieEtAl1987}, and cats \cite{RackWestbury1969}. But as technology develops and computational power increases, researchers have started to use computational models to conduct experiments that may not be feasible to perform \textit{in vivo}. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{muscle_shapes.pdf}
...
From the definition of muscle architecture (see Section \ref{sec:intro_muscle_architecture})---and by simply looking at our muscles---we observe that muscle deformation occurs across all three dimensions of Euclidean space. Thus, two categories of computational models are found in the literature: 1D models (such as Hill-type models) based on ordinary differential equations (ODEs) where the focus is on simplicity and macroscopic behaviour, and 3D PDE-based models where the focus is on detailed spatio-temporal information. Two-dimensional (2D) PDE-based models can be seen as simplifications of 3D models in which deformation in the out-of-plane direction is simply neglected. However, the modelling costs of this physiological simplification (mainly, the ``constant area'' assumption as the 2D version of incompressibility \cite{BaskinPaolini,EpsteinHerzog2003}) outweigh the computational benefits this reduction in dimensionality brings, so they will not be discussed any further in this work.
...
These differences may be attributed to features of skeletal muscle that are not present in these 1D models, such as mass and inertia, shape changes (bulging), variable stresses and strains within the muscle-tendon unit, fatigue, and the overall heterogeneity of muscle tissue, to name a few. 

In particular, the effects of mass and inertia have been studied by \dots

Should include some comments on generic deformation $\rho_0 \*u_{tt} = \Divt{\*P} + \*f$ and reduction to 1D if this is left as a section in the Introduction.
