...
\begin{equation}
    ...
    \right.
\end{equation}
where $T := 0.2 + 0.1/\depsilon_M^*$ \unit{s}, as well as the activation profile:
\begin{equation}
    a(t) = \left\{
        \begin{aligned}
            &0, &&0 \leq t < 0.1, \\
            &5\, a_{max} \, (t-0.1), &&0.1 \leq t < 0.2, \\
            &a_{max}, && 0.2 \leq t \leq T.\\
        \end{aligned}
    \right.
\end{equation}

Several quantities of interest were measured at the end of the simulation time, including values of mean muscle velocity, mean fibre velocity (necessary to compute gearing), muscle force (x component, at the +x end of the muscle block), and mean pennation angle. We also measured the bulging in the midsection of the muscle block perpendicular to the x-axis by computing, in the current configuration, the distance between the midpoints that defined the width and height of the block in the reference configuration (see Figure \ref{fig:bulging_diagram}), that is, if $\*U = \langle U_1,U_2,U_3 \rangle$ is the computed displacement vector field, the height $H_{mus}^T$ and the width $W_{mus}^T$ in the deformed configuration are given respectively by: 
\begin{equation}
...
We implemented this contraction in \texttt{Flexodeal Lite} for multiple whole-muscle shortening speeds $\depsilon_M^* = 0.5, 1, 2, 3, 4, \dots, 13$ \unit{s^{-1}} and multiple maximum activation levels $a_{max} = 0.1, 0.2, \dots, 1.0$. This resulted in 140 computational experiments. A sample of one of these experiments is portrayed in Figure \ref{fig:traces_s2_a10}. The time step size $\Delta t$ (in s) was set to:
\begin{equation}
    \Delta t = \left\{
        \begin{aligned}
            &0.025, &&0 \leq t < 0.1, \\
...
First, in Table \ref{tab:prescribed_strain_rate_mmv}, we show the full list of prescribed strain ranges $\depsilon_M^*$ (in \unit{s^{-1}}) and their equivalent in \unit{mm \ s^{-1}}, which we call the ``prescribed (shortening) velocity'' and can be computed as $L_{mus} \, \depsilon_M^*$, where $L_{mus}$ is the initial length of the muscle block. Furthermore, we compare these values with the mean muscle velocity (MMV) $-|\mathrm{avg}_S(\*V)|$, where we observe that  $|\mathrm{avg}_S(\*V)| \approx \frac{1}{2} L_{mus} \, \depsilon_M^*$, which allows us to replace the prescribed strain rate by the MMV as one of the independent variables of the problem (the other one being activation).

In Figure \ref{fig:g_mfv_force} (left), we show how the mean fibre velocity (MFV) and muscle force (x component, measured at the +x end of the muscle block) vary with activation and MMV. For a given maximum activation level, we observe how MFV increases with increasing MMV at a ratio that is not constant. Then, In Figure \ref{fig:g_mfv_force} (right), we observe how muscle force (x component, measured at the +x end of the muscle block) increases with increasing activation but decreases with increasing MMV, which is expected given the force-velocity relationship of muscle fibres (see, for example, Figure \ref{fig:FL_curves_diff_act}). 
...
\begin{figure}
    ...
    \caption{Mean pennation angle (left) and its rate of change (right) as functions of MMV and maximum activation.
    \label{fig:g_pennation}}
\end{figure}

Next, we compute muscle gearing as described in \eqref{eq:def_gearing} and display first this quantity as a function of force and MMV in Figure \ref{fig:g_elevated}, and as a function of force only for different maximum activation levels in Figure \ref{fig:g_gearing_v_force}. We notice in particular how gearing decreases with increasing force, but increases with MMV and activation. The gearing observed in this case align well with those presented in Holt et al. \cite{Holt2016}. Finally, in Figure \ref{fig:g_bulging} we show how the muscle width and thickness vary in the midsection of the muscle block in an asymmetric fashion, For instance, when the MMV is small, the width of the midsection increases and its thickness decreases with increasing activation. Furthermore, there is a clear inverse relationship between muscle force increases (Figure \ref{fig:g_mfv_force}) and muscle thickness (Figure \ref{fig:g_bulging}). The results are consistent with previous literature findings \cite{Azizi2008VariableGearing}. All simulation shown here had volume changes of < 1\%, consistent with the (nearly) incompressible nature of skeletal muscle tissue \cite{BaskinPaolini}.
