...
First, we introduce the context in which multibody models are unstable and the mathematical reasons behind. Then, we perform some numerical experiments to visualize the effects of this instability. Next, we propose a simple method to stabilize the system based on previously-validated three-dimensional models. This will require us to explore the continuum limit of the mass-enhanced model, that is, as the number of masses in the system grows infinitely. Finally, we visualize the dynamics on this updated system and the implications when analyzing the mass effects in a one-dimensional context.
...
The force-length (FL) relationship, which was described for the first time in 1966 for the semitendinosus muscle in frogs \cite{GordonHuxleyJulian1966}, is an important component of Hill-type muscle models. It describes the forces generated by the active and passive elements in a muscle fibre. 
...
It is clear that the zero solution (i.e. $\xi = (\vec{0},\vec{0})$) is an equilibrium point of the system \eqref{eq:odes_hill_only_first_order} since, from \eqref{eq:def_stretches_stability}, we observe that $\lambda_1 = \dots = \lambda_N = \lambda_M$ and therefore $\vec{g}(\vec{0},\vec{0}) = \vec{0}$. According to the theorem of Hartman-Grobman \cite{HartmanGrobman}, we may linearize \eqref{eq:odes_hill_only_first_order} around this equilibrium point to analyze its stability. Thus, we analyze the linear system:
\begin{equation}
...
\[
    ...
    \end{aligned}
\]
with $F_\lambda = \frac{dF}{d\lambda}$ representing the derivative of the total force curve. In particular, when $(\vec{u}, \vec{v}) = (\vec{0}, \vec{0})$, we have
\begin{equation}
...
\end{equation}
Therefore, to find the eigenvalues $\omega_k$ of $J_g(\vec{0},\vec{0})$, we may use the following identity:
\[
    \det \left(\omega \*I_{2(N-1)} - J_g(\vec{0}, \vec{0}) \right) = \det \left(\omega^2 \*I_{} - \dfrac{F_\lambda(\lambda_M) (N-1)}{l_s^{opt} m_{mus}} \*D_0\right),
\]
which gives rise to two cases, depending on the sign of $F_\lambda(\lambda_M)$. 

First, if $F_\lambda(\lambda_M) > 0$ (that is, the muscle is contracted on the \textit{ascending} limb of the FL relationship), then the eigenvalues of the Jacobian are:
\begin{equation}
...
In contrast, if $F_\lambda(\lambda_M) < 0$ (that is, the muscle is contracted on the \textit{descending} limb of the FL relationship), then the eigenvalues are purely real:
\begin{equation}
