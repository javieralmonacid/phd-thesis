...
{\color{blue}However, because the complexity of the full 3D nonlinear model obscures the main details of the numerical algorithms in use, we therefore focus on applying these methods to a simpler (yet representative) model of one-dimensional Neo-Hookean deformation.}
...
While numerical methods such as Newmark's or HHT-$\alpha$ do verify characteristics 1-6 above in the context of linear problems, they may not for nonlinear problems. {\color{blue}Consider the nonlinear second order ODE:
\[
...
\end{multline}
for any $w \in H_0^1(\Omega)$, where the stretch for the current Newton iteration is $\lambda_k^{n+1} = \partial_x u_k^{n+1} + 1$. {\color{blue}Note that, unlike the nonlinear formulation \eqref{eq:rothe_first_order_VF}, for $k\geq 1$, the linear formulation \eqref{eq:rothe_first_order_lin_2} is symmetric (i.e., test and trial spaces are the same).}
...
{\color{blue}The purpose of this subsection is to help establish both notation and details of the FEM applied to a local problem. This will be useful when comparing time stepping schemes.}
...
{\color{blue}\begin{remark}[Implementation of boundary condtions]
...
Let us consider $\kappa = 1 \ \text{GPa}$, $\nu = 0.499$, $\rho_0 = 1060 \ \text{kg} \, \text{m}^{-3}$, $L = 500 \ \text{mm}$, $T = 500 \text{ms}$, and a boundary condition $g(t) = 0.1t^3$. {\color{blue}The particular choice of these parameters is meant to resemble those found in skeletal muscle. In particular, the Poisson's ratio being close to 0.5 represents the quasi-incompressibility of skeletal muscle \cite{BaskinPaolini}.}
In this case, while the exact solution to \eqref{eq:elasticity_linear} could be computed using Fourier series, we cannot reliably use this expression as any truncation of this series will lead to high oscillations at the ends of the interval (Gibbs phenomenon). Therefore, we decide to compute an ``exact'' solution by taking a sufficiently small $\Delta t$ or $\Delta x$. We emphasize that both the exact solution and its approximations will be computed using the same solver (i.e. an \textit{intrasolver} study). {\color{blue}We avoid intersolver comparisons due to the choice of material, as it can potentially cause the solver to ``lock'', i.e. there will be a point where errors will stop decreasing regardless of how small $\Delta x$ is taken \cite{BabushkaSuri1992}.}
...
\end{equation}
$\kappa$ is the bulk modulus, and $\nu$ the Poisson ratio. Furthermore, consider a rod with $\rho_0 = 1060$ \si{kg.m^{-3}}, $\kappa = 1$ \si{GPa}, $\nu = 0.4$, $L = 100$ \si{mm}, and a simulation end time $T = 2$ s. {\color{blue}Note that we have chosen $\rho_0$ and $\kappa$ as in Section \ref{sec:neohookean_convergence_studies}, but we have reduced the value of $\nu$ to obtain more interpretable results.}
...
{\color{blue}The solver \texttt{ode23tb} gave the best results. Not only it took
...
{\color{blue}The choice of a first-order over a second-order method is also justified by the fact that one way to reduce the spurious oscillations in Newmark's methods is to take $\gamma > 1/2$, at the expense of losing second-order convergence \cite[Remark X.X]{raviartthomas}.}
...
{\color{blue}These suggest a first-order implicit method is a good time stepping scheme candidate for the dynamic version of the full 3D model of skeletal muscle deformation.}
...
\end{equation}
Then, we denote the set on which the spatial coordinates live as $\B_t := \bm{\varphi}(\B_0, t)$, which we call the \textit{current} configuration of the body with reference configuration $\B_0$. Note that, at time $t=0$, we have the consistency condition $\*x = \*X$.
...
Next, we denote the \textit{material gradient} of a scalar field $\F(\*X,t)$ and the \textit{material divergence} of a material vector field $\FF(\*X,t)$ respectively as\footnote{The Einstein summation convention will be used throughout this Chapter.}:
\begin{equation}
\nabla_0 \F(\*X,t) := \pder{\F(\*X,t)}{\*X}, \quad \Divs{\FF(\*X,t)} := \pder{\F_i(\*X,t)}{X_i}.
\end{equation}

Similarly, we denote the \textit{spatial time derivative of a spatial field} $f(\*x,t)$ by $\pder{f(\*x,t)}{t}$. We also define its \textit{spatial gradient} and the \textit{spatial divergence} of a vector field $\*f(\*x,t)$ by:
\begin{equation}
...
We define the \textit{left} and \textit{right Cauchy-Green tensors} respectively as
\begin{equation}
...
\end{equation}
where $\*a_0$ is a unit vector describing the initial orientation of the material line element. Given that $\*a_0$ can be imagined as the orientation of a \textit{fibre} passing through $(\*X,t) \in \B_0$, we refer to $\lambda(\*X,t)$ as the \textit{fibre stretch}. 

We also define the \textit{fibre strain rate} as the time derivative of the fibre stretch, normalized by a factor $\depsilon_0$ representing the maximum strain rate at which the material element can deform:
...
\end{equation}
where the last equality arises from taking an implicit time derivative in \eqref{eq:def_lambda} and the fact that $\dot{\*C} = 2\*F^\T \*d \*F$ (cf. \cite[Eq. (2.168)]{HolzapfelBook}). 
...
Moreover, this multiplicative decomposition also yields a \textit{modified fibre stretch} $\bar{\lambda}$:
\begin{equation} \label{eq:def_lambda_bar}
\bar{\lambda} := \sqrt{\*a_0^\T \bar{\*C} \*a_0} = J^{-1/3} \lambda,
\end{equation}
as well as a \textit{modified fibre strain rate}:
...
\end{equation}
It is also called the \textit{deviatoric tensor} since, for example,
\begin{equation}
...
\end{equation}
In turn, the isochoric part, $\C_{iso}$, can be computed as (cf. \cite[Example 6.8]{HolzapfelBook}):
\begin{equation}
...
\subsection{SEF for compressible, transversely isotropic materials}

In compressible, transversely isotropic materials (i.e., materials that are reinforced by only \text{one} family of fibres and that have a \text{single} preferred direction of deformation), the following general SEF can be established \cite{HolzapfelBook,Weiss1996}:
...
Let us consider skeletal muscle tissue as a fibre-reinforced (i.e. transversely isotropic), composite, nearly incompressible hyperelastic material \cite{Seba, Hadi, Paper1_WakelingEtAl2020}. Furthermore, let us postulate the following SEF for skeletal muscle tissue:
\begin{equation}
...
\end{equation}
where $\kappa$ is the bulk modulus of muscle tissue. In this case, the bulk modulus may appear as a single constant or as a weighted combination of bulk moduli of the different tissue components (such as intramuscular fat, ECM, and cellular material; see below). In turn, the isochoric component of $\Psi$ takes the form:
...
{\color{red}We assume that $\Psi_{fibre}$ is given implicitly by the equation:}
...
\end{equation}
where $\chi_{ecm} > 0$ is the fraction of ECM content in muscle. Furthermore, the SEFs for ECM and cellular material are given respectively by:
...
\begin{subequations} \label{eq:LF_main_system}
    ...
    \end{align}
\end{subequations}
Here, the first expression corresponds to the definition of the material velocity (see Remark \ref{re:velocity_def}), whereas the second equation is just Cauchy's first equation of motion written in the Lagrangian description (see \cite[Section 4.3]{HolzapfelBook}). In the third equation, $J(\*U)$ can be considered as the \textit{true} dilation whose approximation is given by $D$. Furthermore, $\rho_0$ denotes the \textit{reference density} of the MTU, and although it might depend on space coordinates $\*X$, it does not depend on time.
...
We now derive in detail the linearization of the nonlinear formulation \eqref{eq:LF_weak}. First, we give general details of Newton iteration. Then, we compute each one of the blocks forming the tangent operator. Finally, we focus on the computation of the fibre contribution to the fourth-order tensor $J\c$ since it is an expression that is typically not reported in the literature.
...
To compute the fourth-order elastcity tensor $\c^{n+1}$, we first need to find the material tensor $\bar{\C}$ (see Equations \eqref{eq:def_c}, \eqref{eq:Jc_iso}, and \eqref{eq:def_c_bar}). Furthermore, each contribution to isochoric SEF $\Psi_{iso}$ yields a different contribution to the elasticity tensor $\bar{\C}$. For example, if we assume that $\B_0$ contains muscle fibres, base material, and intramuscular fat as in \eqref{eq:def_Psi_iso_fibre_bm_fat}, we will have:
\begin{equation}
...
The derivation of all parts that make the $\bar{\C}$ tensor follows the same structure, so we will only provide details for the fibre contribution. First, according to \eqref{eq:def_C_bar}, we have
\begin{equation}
...
\begin{align}
    ...
    &\hspace{9em}+ \sigma_{0,mus} a^{n+1} \widehat{\sigma}_L(\bar{\lambda}^{n+1}_k) \der{\sigma_{V}(\bar{\depsilon}^{n+1}_k)}{\bar{\depsilon}^{n+1}_k} \pder{\bar{\depsilon}^{n+1}_k}{\bar{\lambda}^{n+1}_k} \dfrac{1}{2\bar{\lambda}^{n+1}_k}.
\end{align}
...
where
\begin{align}
    \mathfrak{C}\left(\bar{\lambda}^{n+1}_k, \bar{\depsilon}^{n+1}_k \right) = &- \dfrac{2}{\left( \bar{\lambda}^{n+1}_k \right)^4} \, \sigma_{0,mus} a^{n+1} \widehat{\sigma}_L(\bar{\lambda}^{n+1}_k) \widehat{\sigma}_V(\bar{\depsilon}^{n+1}_k) \\
    ...
    \notag &- \dfrac{2}{\left( \bar{\lambda}^{n+1}_k \right)^4} \, \sigma_{0,mus} \widehat{\sigma}_P(\bar{\lambda}^{n+1}_k) \\
    &+ \dfrac{1}{\left( \bar{\lambda}^{n+1}_k \right)^3} \, \sigma_{0,mus} \der{\widehat{\sigma}_P}{\bar{\lambda}} \left( \bar{\lambda}^{n+1}_k  \right)
\end{align}
In this way,
\begin{equation}
