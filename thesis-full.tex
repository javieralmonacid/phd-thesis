...
\end{equation}
where $\bar{\*S}$ is the \textbf{ficticious PK2 stress tensor}:
...
\end{equation}
where $\bar{\btau}$ is the \textbf{ficticious Kirchhoff stress tensor}:
...
\end{equation}
where the \textbf{ficticious elasticity tensor} $\bar{\C}$ in the material description is given by
...
\c = \c_{vol} + \c_{iso},
\end{equation}
where the volumetric part, $\c_{vol}$ can be written as
\begin{equation}
J \c_{vol} = pJ(\*I \otimes \*I - 2p\mathbb{I}) + J^2 \der{^2 \Psi_{vol}(J)}{J^2} \*I \otimes \*I.
\end{equation}
In turn, the isochoric part, $\c_{iso}$ can be computed as
...
\end{equation}
where $\p$ is the spatial projection tensor defined in \eqref{eq:def_projection_spatial} and the \textbf{ficticious elasticity tensor} $\bar{\c}$ in the spatial description is given by
\begin{equation} \label{eq:def_c_bar}
...
\end{equation}
where $\chi_{fat}$ is the fraction of intramuscular fat present in muscle tissue. In terms of stresses these SEFs induce, from Section \ref{sec:cons_eqs_PK2_kirchhoff_general} (and in particular from the definition of the isochoric Kirchhoff stress in \eqref{eq:def_tau_iso}), we notice that an expression for $\btau_{iso}$ can be found by computing first the ficticious PK2 stress $\bar{\*S}$ \eqref{eq:def_S_bar}, and then the ficticious Kirchhoff stress $\bar{\btau}$ by pushing-forward $\bar{\*S}$ (see Equation \ref{eq:def_tau_bar}). This will be the main idea of the following subsections.
...
\section{Extending the SEF to the muscle-tendon unit}
...
\end{equation}
where $\bchi_M(\*X)$, $\bchi_A(\*X)$, and $\bchi_T(\*X)$ are indicator functions on $\B_0^M$, $\B_0^A$, and $\B_0^T$, respectively. Furthermore, the bulk modulus $\kappa$ takes the form
\begin{equation}
...
\subsection{A first order, fully implicit time discretization}

\subsection{Comparison with the Eulerian formulation}

\subsection{Linearization, elasticity tensors}
