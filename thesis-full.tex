...
As a prelude to the development of computational tools for the study of 3D skeletal muscle deformation, we now face the task of finding an appropriate time stepping scheme for the dynamic problem, in an effort to extend the developments of Rahemi \cite{Hadi} and Dominguez \cite{Seba} regarding quasi-static models. To accomplish this, we explore different time discretization alternatives for one-dimensional Neo-Hookean deformation. This will provide us with a simple (yet representative) model to test different numerical schemes that can be later considered for the three-dimensional problem (see Chapter \ref{ch:lagrangian_formulation}).

First, we discuss different time stepping methods that are commonly used in dynamic analysis of structures, as well as their advantages and disadvantages. Next, we state the model problem and present these methods in two different contexts, depending on which variable in the continuous problem: time or space. This is a particularly important consideration for nonlinear problems with (potentially) nonsmooth solutions. Then, we present some convergence studies for a linear problem and perform experiments of Neo-Hookean deformation. In particular, we study a series of time discretizations, from methods used in engineering finite element analysis to Matlab's built-in solvers.  We finish the chapter with a discussion on the obtained results and what this means for the development of a 3D \textit{dynamic} finite element code for skeletal muscle tissue.
...
While it is clear for linear problems to verify characteristics 1-6 above, it not the case for nonlinear problems. For example, at each time step, the nonlinear problem is linearized using strategies such as Newton's method. Thus, if iterative solvers are used to solve the (discretized) linear problem, then multiple linear systems are solved at each Newton iteration, and multiple Newton iterations are performed at each time step. Therefore, second-order accuracy might not be achieved, or if it does, it might still exhibit overshooting and oscillations beyond the high-frequency regime \cite{ErlicherBonaventuraBursi2002}.
...
One alternative is to discretize the time variable first while keeping the space variable continuous. This is referred to as \textit{Rothe's method}. For second order parabolic and hyperbolic systems (the latter being the case for elastodynamics), this means that, at each time step, we are bound to solve an elliptic PDE in the space variable. For nonlinear PDEs, the linearization can be performed at a continuous level using, for instance, Gateaux derivatives. One of the advantages of this method is that, in the context of finite element methods, updates to the mesh can easily be perform since they do not alter the variational formulation of the elliptic problem. However, one major disadvantage is that it is harder to test different time stepping strategies since they involve modifying the structure of the variational formulation, which consequently induces significant modifications to the computational solvers.

A different approach is to discretize the space variable first, then the time variable. This is known as the \textit{method of lines  (MOL)} and it is how Newmark-type methods are typically introduced in the literature \cite{HughesBook}. In this case, the parabolic or hyperbolic PDE is transformed into a system of ODEs, for which multiple solvers exist in the literature. This is one of the major advantages of the MOL, however, this also means that the spatial mesh is fixed at all times, which, depending on the physiological application, may not be an ideal choice.
...
We now consider different time stepping schemes for the initial boundary value problem \eqref{eq:elasticity_1d} using different approaches. The space discretization will remain the same throughout and will be based on second-order Lagrange finite elements.
...
\begin{subequations} \label{eq:rothe_first_order}
    ...
    \end{gather}
\end{subequations}
where $\lambda^{n+1} = \partial_x u^{n+1} + 1$. Combining these two equations into a single one for $u^{n+1}$ reads:
...
\end{equation}
After some algebraic manipulations, the linear problem \eqref{eq:rothe_first_order_lin_1} reads: find Newton increments $du_k^{n+1} \in H^1(\Omega)$ such that $du_0^{n+1} = 0$ on $x=0$, $du_0^{n+1} = g(t^{n+1}) - g(t^{n})$ on $x=L$, $du_k^{n+1} = 0$ on $x=0,L$ for k > 0, and:
...
\end{multline}
for any $w \in H_0^1(\Omega)$, where $\lambda_k^{n+1} = \partial_x u_k^{n+1} + 1$. The linear problem \eqref{eq:rothe_first_order_lin_2} is solved for each $k \geq 0$ and the displacement is updated as $u_{k+1}^{n+1} = u_k^{n+1} + du_k^{n+1}$ until some convergence criterion is satisfied. 
...
\]
from which we obtain the following identities that will help us move from the an arbitrary interval $T$ to the reference interval $\widehat{T}$:
\begin{gather}
    \der{\widehat{u}}{\widehat{x}} = B \der{u}{x}, \\
...
Given that we are looking for functions in the finite element space \eqref{eq:fespace_1d}, we can write any element $u \in \mathbb{P}_2(T)$ 
as
...
\]
Furthermore, the mapped expression $\widehat{u} = u \circ \mathcal{F}$ can be written as
\begin{equation}
...
\]
with an analogous expression for the derivative of $u$.
...
\end{multline}
where each term can be fully computed following the ideas introduced above (we drop the $h$ subscript for better readability and recall that $B=x_{i+1}-x_i$). First, the terms on the left-hand side of \eqref{eq:rothe_first_order_lin_4}:
\begin{align}
...
\end{equation} 
where $V_h$ is an appropriate finite-dimensional subspace of $H_0^1(\Omega)$. As before, we consider $V_h$ given by \eqref{eq:fespace_1d} and a discretization $\mathcal{T}_h$ of the interval $[0,L]$, i.e. $\mathcal{T}_h = \{ [x_i,x_{i+1}] \}_{i=0}^N$. In this case, for $T \in \mathcal{T}_h$, we can write
\begin{equation}
...
\end{equation}
but now $\*K(\*U)$ is the ``stiffness vector'':
...
The equation above is a nonlinear system of equations that can be rewritten as:
 ...
 \begin{equation}
     \*R(\*U^{n+1}_{k+1}) = \*R(\*U^{n+1}_k) + \pder{\*R}{\*U^{n+1}}(\*U_k^{n+1}) \Delta \*U_{k+1}^{n+1} + \dots = \*0,
 \end{equation}
 that is,
 \begin{equation}
     \pder{\*R}{\*U^{n+1}}(\*U_k^{n+1}) \Delta \*U_{k+1}^{n+1} = -\*R(\*U_k^{n+1}).
 \end{equation}
 Therefore, the Newton iteration for the nonlinear problem \eqref{eq:mol_first_order_nonlinear} reads: find $\Delta \*U_{k+1}^{n+1}$ such that:
 \begin{equation} \label{eq:mol_first_order_linear}
     \left[ \*M + \Delta t^2 \pder{\*K}{\*U}(\*U_k^{n+1}) \right] \Delta \*U_{k+1}^{n+1} = - \*M \*U_k^{n+1} - \Delta t^2 \*K(\*U_k^{n+1}) + \*M(\*U^n + \Delta t \*V^n).
 \end{equation}
Then, we can update the displacements $\*U_{k+1}^{n+1} = \*U_k^{n+1} + \Delta \*U_{k+1}^{n+1}$. The new (matrix) term $\pder{\*K}{\*U}$ can be computed from \eqref{eq:mol_first_order_def_stiffness_vector} as:
...
\begin{subequations} \label{eq:newmark}
    ...
    \end{align}
\end{subequations}
where $\varphi_n = \varphi(t_n, y_n, z_n)$. In terms of accuracy, the scheme is first order if $\gamma \neq 1/2$ and second order if $\gamma = 1/2$. Furthermore, the scheme is unconditionally stable if $1/2 \leq \gamma \leq 2\beta$ (cf. \cite{HughesBook,raviartthomas}). In particular, we will consider $\beta=1/4$ and $\gamma=1/2$, which corresponds to the \textit{average acceleration} method. Some other well-known members of Newmark family of schemes are the \textit{linear acceleration} method ($\beta=1/6$, $\gamma=1/2$), the \textit{Fox-Goodwin (royal road)} method ($\beta=1/12$, $\gamma=1/2$), and the \textit{central differences} method ($\beta=0$, $\gamma=1/2$) \cite{HughesBook}. We note that these are all second-order, single-step methods.

\subsection{Newmark in the context of Rothe's method} \label{sec:rothe_newmark}

As before, we define the velocity $v = u_t$ and the acceleration $a = u_{tt}$, then discretize the nonlinear elasticity equation \eqref{eq:elasticity_1d_1} using the Newmark scheme \eqref{eq:newmark}:
\begin{subequations} \label{eq:rothe_newmark}
    ...
    \end{align}
\end{subequations}
Next, we define the the following ``predictors'' which only require information from the previous time step:
...
\end{equation}
where $e_t$ and $e_t'$ ($e_x$ and $e_x'$) are the errors corresponding, respectively, to different time step (mesh) sizes $\Delta t$ and $\Delta t'$ ($\Delta x$ and $\Delta x'$).
...
\begin{table}
    ...
    \end{tabular} 
    \caption{Error behaviour for each of the proposed methods. Left three columns: convergence in time ($L^\infty$ norm) with fixed space mesh size ($\Delta x = L/8 = 62.5 \ \text{mm}$). Right three columns: convergence in space ($L^2$ norm) with fixed time step size $\Delta t = 1e-05 s$. \label{tab:convergence_linear_elasticty_1d}}
\end{table}
...
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
Therefore, the boundary condition for the elasticity model \eqref{eq:elasticity_1d} is $u(L,t) = L \varepsilon_k(t)$, $k =1,2,3$. First, we discretize the PDE \eqref{eq:elasticity_1d_1} using the six methods mentioned in the previous section with $\Delta x = L/8$, $\Delta t = 10^{-4}$, and tolerances for the nonlinear solver set to $10^{-6}$ in displacement and $10^{-8}$ in the residual.
...
We can easily test an implicit-explicit (IMEX) scheme for the nonlinear system of equations \eqref{eq:mol}. Observe from the definition of $\*K(\*U)$ in \eqref{eq:mol_first_order_def_stiffness_vector} that:
\begin{multline}
...
(\texttt{RelTol} = $10^{-2}$, \texttt{AbsTol} = $10^{-4}$). The MOL equations are solved as a square system of 2 $\times$ (DOF-2) equations. 
...
website:
\begin{itemize}
	\item ode113 is a variable-step, variable-order (VSVO) 
	...
	\item ode113 may be more efficient than ode45 at stringent 
	tolerances or if the ODE function is particularly expensive 
	to evaluate. ode113 is a multistep solver -- it normally 
...
the shortest time to compute a solution but also allowed for a time step
an order of magnitude larger than most of the other stiff solvers. However,
...
\begin{itemize}
	\item ode23tb is an implementation of TR-BDF2, an implicit 
	...
	Like ode23s and ode23t, this solver may be more efficient 
	than ode15s for problems with crude tolerances.
\end{itemize}

We show, in particular, the velocities and the time step sizes for the solvers that finished their computation (according to Table \ref{tab:mol_matlab_solvers}) in Figure \ref{fig:sin_pull_matlab}. Here, we observe that only \texttt{ode23s} and \texttt{ode23tb} computed velocities somewhat similar to those in Figure \ref{fig:sin_pull_vel}, but overshooting was still a problem in these solvers. Moreover, the time step chosen by these solvers was in line with the one considered in Figure \ref{fig:sin_pull_vel}.
