...
Consider the dynamic contraction of a muscle-tendon unit of length $L = L(t)$. Moreover, let $L_M$ and $L_T$ denote the muscle and tendon lengths, respectively, which satisfy $L_M(t) + L_T(t) = L(t)$ at every time $t \geq 0$ (see Figure \ref{fig:mtu_sketch}).
...
\begin{equation} \label{eq:Hill_force}
    F_{Hill}(\lambda, \depsilon) = F_0 \Big\{ a(t) F_A(\lambda_M) F_V(\depsilon_M) + F_P(\lambda_M) \Big\}.
\end{equation}
In this expression:
\begin{enumerate}
    ...
    \end{equation}
    \item $F_A = F_A(\lambda_M)$ is known as the active force-length relationship, whereas $F_V = F_V(\depsilon)$ is known as the force-velocity relationship. They are related to the CE of the model and describe the force produced due to actin-myosin interactions during activation; 
    \item $F_P = F_P(\lambda_M)$ is known as the passive force-length relationship. It is related to the PEE and describes the force produced by passive components of the muscle fibre (such as titin), which act to prevent overstretching of the sarcomeres.
\end{enumerate} 
...
\end{equation}
Where $l_{MTU}^{opt}$ is the optimal MTU length (given) and $r_{mus}$ is the muscle-to-MTU-length ratio (see Table \ref{tab:1d-parameters}). Ideally, $l_{mus}^{opt}$ should be such that (inactive) muscle and tendon forces balance, i.e. $F_M = F_T$ (see, for example, Figure \ref{fig:muscle_models_1D}A). However, because different studies use different analytical expressions for the force relationships in Figure \ref{fig:force_curves_1D}, we must think of this estimate as a deviation $\Delta l_{mus}^{opt}$ from the true value $l_{mus}^{opt}$, i.e.
\begin{equation}
...
In a similar fashion, to ensure the simulation begins from static equilibrium, we assume that the given initial MTU length $L(0)$ only provides estimates for the initial muscle and tendon lengths, that is:
\begin{equation}
